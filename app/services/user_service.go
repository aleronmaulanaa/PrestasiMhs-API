...
type UserService interface {
	...
	GetStudentByID(c *fiber.Ctx) error              // [NEW]
    GetLecturerAdviseesSRS(c *fiber.Ctx) error      // [NEW]
}
...
func (s *userService) GetStudentByID(c *fiber.Ctx) error {
	studentID := c.Params("id")
	student, err := s.repo.FindStudentByID(studentID)
	if err != nil {
...
// 2. GET /api/v1/lecturers/:id/advisees
// @Summary      Get Lecturer Advisees
// @Description  Melihat daftar mahasiswa bimbingan dosen tertentu
// @Tags         Students & Lecturers
// @Produce      json
// @Security     BearerAuth
// @Param        id   path      string  true  "Lecturer Table ID"
// @Success      200  {array}   models.StudentResponse
// @Router       /lecturers/{id}/advisees [get]
func (s *userService) GetLecturerAdviseesSRS(c *fiber.Ctx) error {
	lecturerID := c.Params("id")
	students, err := s.repo.FindStudentsByAdvisorID(lecturerID)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"status": "error", "message": err.Error()})
	}
	return c.JSON(fiber.Map{"status": "success", "data": students})
